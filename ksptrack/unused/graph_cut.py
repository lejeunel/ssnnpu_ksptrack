...
class graph_cut:
    ...
    def __init__(self,conf,fg_pm,bg_pm,scores_ksp,labels,gt,gamma=None,lambda_=None):
        self.conf = conf
        self.fg_pm = fg_pm
        self.labels = labels
        ...
        #If no bg model,
        if(bg_pm is None):
            self.bg_pm = self.get_comp_scores_ksp(self.labels) #Will call calc_bagging
        else:
            self.bg_pm = bg_pm
    ...
    def get_scores(self):
        ...
        conf_mat = metrics.confusion_matrix(self.gt.ravel(), self.gc_maps.ravel())
    ...
    def run(self):
        ...
        self.gc_scores = None
        print("(gamma,lambda_) = (" + str(self.gamma) + ',' + str(self.lambda_) + ")")
        with progressbar.ProgressBar(maxval=len(self.conf.frameFileNames)) as bar:
            ...
            #for j in np.array([10,20]):
                ...
                this_fg_costs = -np.log(this_fg + small) / -np.log(
                    small) - self.gamma * np.log(this_pm_fg + small) / -np.log(small)
                this_bg_costs = -np.log(this_bg + small) / -np.log(
                    small) - self.gamma * np.log(this_pm_bg + small) / -np.log(small)
                ...
                inds = np.arange(self.scores_ksp[j, :, :].size).reshape(self.scores_ksp[j, :, :].shape)
                horz = np.c_[inds[:, :-1].ravel(), inds[:, 1:].ravel()]
                vert = np.c_[inds[:-1, :].ravel(), inds[1:, :].ravel()]

                orig_image = color.rgb2gray(utls.imread(self.conf.frameFileNames[j]))
