...
def get_scores_rates(sets,
    ...
    scores = fn.score_path_sets(sets,
                                labels,
                                's',
                                't',
                                set_idx=winInd,
...
def makeFrameFileNames(frame_prefix, seqStart, seqEnd, frameDigits, frameDir,
                       root_path, ds_dir, frame_extension):
...
def readCsv(csvName, seqStart, seqEnd):
    return np.loadtxt(open(csvName, "rb"), delimiter=";",
                      skiprows=5)[seqStart:seqEnd + 1, :]
...
dataOutDir = getDataOutDir(c.dataOutRoot, c.ds_dir, c.resultDir,
                           c.out_dir_prefix, c.testing)

#Make frame file names from seqStart and seqEnd
gtFileNames = makeFrameFileNames(c.frame_prefix, c.seqStart, c.seqEnd,
                                 c.frameDigits, c.truth_dir, c.root_path,
                                 c.ds_dir, c.frame_extension)

frameFileNames = makeFrameFileNames(c.frame_prefix, c.seqStart, c.seqEnd,
                                    c.frameDigits, c.frameDir, c.root_path,
                                    c.ds_dir, c.frame_extension)
...
centroids_loc = pd.read_pickle(
    os.path.join(precomp_desc_path, 'centroids_loc_df.p'))
labels = scipy.io.loadmat(os.path.join(labelMatPath,
                                       'sp_labels_tsp.mat'))['sp_labels']
npzfile = np.load(os.path.join(precomp_desc_path,
...
seen_feats_df = pd.read_pickle(
    os.path.join(precomp_desc_path, 'seen_feats_df.p')).T
    ...
    for j in np.asarray([25]):
        ...
        this_gc = np.logical_not(
            cut_from_graph(edges, unaries.reshape(-1, 2),
                           pairwise).reshape(scores[j, :, :].shape))
        gc_maps[j, :, :] = this_gc
...
ind = 25
plt.subplot(221)
plt.imshow(color.label2rgb(gt[ind, :, :], my.imread(frameFileNames[ind])))
plt.subplot(222)
