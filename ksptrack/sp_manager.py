...
class SuperpixelManager:
    ...
    def __init__(self, dataset, conf, direction='forward', with_flow=False):
    ...
    def make_init_dicts(self):
        ...
        file_dict = os.path.join(self.conf.dataOutDir,
                                 'init_constraint_' + self.direction + '.p')
        if (not os.path.exists(file_dict)):
            ...
            with progressbar.ProgressBar(maxval=len(frames)) as bar:
                for i in range(len(frames)):
                    ...
                    combined = utls.df_crossjoin(
                        df1, df2, suffixes=('_in', '_out'))
                    combined['diff_x'] = combined['pos_norm_x_in'] - combined[
                        'pos_norm_x_out']
                    combined['diff_y'] = combined['pos_norm_y_in'] - combined[
                        'pos_norm_y_out']
                    combined['dist'] = np.sqrt(combined['diff_x']**2 +
                                               combined['diff_y']**2)
    ...
    def apply_overlap_constraint(self):
        ...
        if (not os.path.exists(file_dict)):
            ...
            dict_.update({(frames[-1], l): (frames[-1], [])
                          for l in np.unique(self.labels[..., frames[-1]])})
            ...
            self.logger.info(
                'Loading overlap dictionary... (delete to re-run)')
            with open(file_dict, 'rb') as f:
                return pk.load(f)
        self.dict_init = dict_
        return dict_
    ...
    def make_hoof_dict(self):
        ...
        if (not os.path.exists(file_dict)):
            ...
            #if(self.direction == 'forward'):
            fx = np.concatenate(
                (flows[keys[0]], flows[keys[0]][..., -1][..., np.newaxis]),
                axis=-1)
            fy = np.concatenate(
                (flows[keys[1]], flows[keys[1]][..., -1][..., np.newaxis]),
                axis=-1)
            #else:
            ...
            with progressbar.ProgressBar(maxval=len(frames)) as bar:
                for f, p in zip(frames, range(len(frames))):
                    ...
                    l_mask = [
                        self.labels[..., f].ravel() == l for l in unq_labels
                    ]
                    ...
                    hoof_ = np.asarray(
                        [[np.sum(norm[l_ & b_]) for b_ in b_mask]
                         for l_ in l_mask])

                    # Normalize w.r.t. L1-norm
                    l1_norm = np.sum(
                        hoof_, axis=1).reshape((unq_labels.size, 1))
                    hoof_ = np.nan_to_num(hoof_ / l1_norm)

                    # Store HOOF in dictionary
                    hoof[f] = {
                        unq_labels[i]: hoof_[i, :]
            ...
            with progressbar.ProgressBar(maxval=n_keys) as bar:
                for k, i in zip(self.dict_over.keys(),
                    ...
                    for c in self.dict_over[k][1]:  # candidates
                        ...
                        end_s = c
                        inter_hoof = utls.hist_inter(hoof[start_f][start_s],
                                                     hoof[end_f][end_s])
