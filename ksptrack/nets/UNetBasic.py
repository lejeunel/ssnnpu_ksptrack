...
class UNetBasic(UNetImpl):
    ...
    def train(self,
        ...
        imgs = UNetImpl.preprocess_imgs(self,
                                        im_list,
                                        input_y,
                                        input_x,
                                        n_chans)
        ...
        n_samp_train = np.floor(len(im_list)*(1-conf.unet_validation_split)).astype(int)
        ...
        self.gausStdForDataAugm = (conf.unet_data_gaussian_noise_std / std_train)
        ...
        if (save_examples):
            ...
            for _ in datagenTrain.flow(imgs_train[idx:(idx + 1), ...], batch_size=1, save_to_dir=self.dataAugPreviewDir,
        ...
        if conf.unet_data_use_generator:
            ...
            trainGenerator = datagenTrain.flow(imgs_train, gts_train,
                                               batch_size=conf.unet_batchsize,
                                               shuffle=True)

            valGenerator = datagenTrain.flow(imgs_val, gts_val,
                                             batch_size=conf.unet_batchsize,
                                             shuffle=True)
    ...
    def eval(self,
        ...
        imgs = UNetImpl.preprocess_imgs(self, im_list, self.inputDimY, self.inputDimX, self.nbrChannels)
