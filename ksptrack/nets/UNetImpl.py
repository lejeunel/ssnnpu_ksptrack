...
class UNetImpl:
    def __init__(self,
        ...
        self.weightFilePath, self.logFilePath, self.dataAugPreviewDir = utls.get_file_paths(
            unet_path, conf)
        ...
        if img_height == None:
            ...
            if (float(img_height) / div_var) % 1 != 0:
                ...
                self.logger.info('Cannot take image height, use ' +
                                 str(img_height) + ' instead')
        ...
        self.logger.info('num. of output channels ' +
                         str(self.nbrChannels_out))
    ...
    def interp_features(self, conf, feat_path, labels, n_jobs=4):
        ...
        out = pd.DataFrame(out, columns=["frame", "sp_label", "desc"])
    ...
    def _train(self,
        ...
        imgs, imgs_mask, mean, std = self.preprocess_and_normalize_imgs(
            im_list, gaze_labels=gaze_labels)
    ...
    def _forward_prop(self, conf, feat_path, im_list, loss_name):
        ...
        div_var = 2**UNetModels.MODEL_DEPTH
        im_feat = np.zeros(
            (len(imgs), int(self.inputDimY / div_var),
             int(self.inputDimX / div_var), UNetModels.FEATURE_DIM),
            dtype='float32')
        ...
        self.logger.info(
            'Start forward propagation of %i images...' % len(imgs))
        while ind_s < len(imgs):
            # apply forward propagation to batch
            im_feat[ind_s:ind_e, ...] = model_get_feat(
                [imgs[ind_s:ind_e, ...]])[0]
        ...
        seq = hd5_file.create_dataset(
            'raw_feat', (im_feat.shape),
            dtype='float32',
            compression='gzip',
            compression_opts=2)
        seq[...] = im_feat
    ...
    def _forward_prop_output(self, conf, im_list, loss_name):
        ...
        model_output = self.model.predict(
            imgs, batch_size=imgs.shape[0], verbose=0, steps=None)
    ...
    def preprocess_imgs(self, im_list, post_height, post_width, nbr_channels):
        ...
        pbar = ProgressBar(
            widgets=[Percentage(), Bar()], maxval=len(im_list)).start()
        for i, item in enumerate(im_list):
            ...
            if img.shape[0] == post_height and img.shape[1] == post_width:
                ...
                imgs_p[i] = scm.imresize(
                    img, (post_height, post_width, nbr_channels),
                    interp='bilinear')
            pbar.update(i)
        pbar.finish()
    ...
    def preprocess_and_normalize_imgs(self, im_list, gaze_labels=None):
        ...
        imgs = self.preprocess_imgs(im_list, self.inputDimY, self.inputDimX,
                                    self.nbrChannels)
        ...
        if gaze_labels == None:
            ...
            imgs_mask = np.array(imgs)
            imgs_mask /= 255  # scale masks to [0,1]
            imgs_mask = np.concatenate((imgs_mask, imgs_gauss),
    ...
    def create_and_compile_model(self,
        ...
        self.modelClass = UNetModels(self.inputDimY, self.inputDimX,
                                     self.nbrChannels, self.nbrChannels_out)

        # generate the requested model
        if (mode == 'features'):
            self.model, self.layerNbr = self.modelClass.gen_model_features()
        else:
            self.model = self.modelClass.gen_model_standard(
                (self.inputDimY, self.inputDimX, self.nbrChannels))
        ...
        if conf.unet_optimizer == 'sgd':
            optimizer = kopt.SGD(
                lr=conf.unet_sgd_learning_rate,
            ...
            optimizer = kopt.Adamax(
                lr=conf.unet_adamax_learning_rate,
        ...
        self.model.compile(
            optimizer=optimizer, loss=loss, metrics=loss_metrics)
