...
for key in types: # Types
    ...
    for f in frames_idx:
        ...
        ims.append(im)
        im = gaze.drawGazePoint(conf.myGaze_fg,
                                f,
                                im,
                                 radius=14)
...
ims_gaze_crop = [util.crop(ims_gaze[i], to_crop_im[i]) for i in range(len(ims))]
...
ims_gaze_res = [(transform.resize(ims_gaze_crop[i], f_size)*255).astype(np.uint8) for i in range(len(ims))]
ksp_res = [(transform.resize(ksp_crop[i], f_size)*255).astype(np.uint8) for i in range(len(ims))]

pw = 10
ims_gaze_pad = [util.pad(ims_gaze_res[i], ((pw,pw),(pw,pw),(0,0)), mode='constant', constant_values=255) for i in range(len(ims))]
...
for i in range(len(ims_pad)):
    io.imsave(os.path.join(file_out,'im_gaze_{0}.png'.format(i)), ims_gaze_pad[i])
