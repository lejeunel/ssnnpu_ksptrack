...
if(not os.path.exists(file_out)):
    ...
    for key in rd.types:
        ...
        for dset in range(len(rd.out_dirs_dict_ksp[key])):
            ...
            gt = l_dataset.gt
            ksp_mean_all = np.load(os.path.join(path_,
                                            'dataset.npz'))['mean_ksp_scores']
...
ksp_flat = [(cmap(ksp_means[i][j])[...,0:3]*255).astype(np.uint8) for i in range(len(ksp_means)) for j in range(dsets_per_type)]
...
to_crop_ksp = [np.ceil((((ksp_flat[i].shape[0]-min_height)/2,(ksp_flat[i].shape[0]-min_height)/2), ((ksp_flat[i].shape[1]-min_width)/2,(ksp_flat[i].shape[1]-min_width)/2),(0,0))) for i in range(len(ksp_flat))]

ims_crop = [util.crop(ims_flat[i], to_crop_im[i]) for i in range(len(ims_flat))]
ksp_crop = [util.crop(ksp_flat[i], to_crop_ksp[i]) for i in range(len(ksp_flat))]

f_size = ims_crop[0].shape

ims_res = [(transform.resize(ims_crop[i], f_size)*255).astype(np.uint8) for i in range(len(ims_crop))]
ksp_res = [(transform.resize(ksp_crop[i], f_size)*255).astype(np.uint8) for i in range(len(ksp_crop))]

pw = 10
ims_pad = [util.pad(ims_res[i], ((pw,pw),(pw,pw),(0,0)), mode='constant', constant_values=255) for i in range(len(ims_res))]
ksp_pad = [util.pad(ksp_res[i], ((pw,pw),(pw,pw),(0,0)), mode='constant', constant_values=255) for i in range(len(ims_res))]

all_ = np.concatenate([np.concatenate([ims_pad[i], ksp_pad[i]], axis=1) for i in range(len(ims_pad))], axis=0)
