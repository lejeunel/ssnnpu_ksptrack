...
def resize_stack(X, shape):
    ...
    n_chans = X[..., 0].shape[-1]
    return np.asarray([
        transform.resize(X[..., i], (shape, shape, n_chans))
        for i in range(X.shape[-1])
    ]).transpose(1, 2, 3, 0)
...
def resize_datasets(X, shape):
    ...
    return np.concatenate([resize_stack(X[i], shape) for i in range(len(X))],
                          axis=3)
...
def get_best_model_path(path_,
    ...
    model_paths = glob.glob(os.path.join(path_, model_fname))

    model_paths = [os.path.basename(f) for f in model_paths]

    regex = r"(\d{2}(?=[^\d]))"
    saved_epochs = np.asarray(
        [int(re.search(regex, p).group(0)) for p in model_paths])

    # Get model with closest num. of epochs
    best_model_idx = np.argmin(
        np.abs(np.tile(best_epoch, (saved_epochs.shape[0], 1)) - saved_epochs))
