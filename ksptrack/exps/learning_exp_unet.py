...
import logging
from nets.UNetImpl import UNetImpl
import re
from nets import utils as netutls
...
def main(confs,
    ...
    if(train):
        ...
        for i in range(n_folds):
            ...
            X = (resize_datasets([datasets[train_folds[j]].X_all_images for j in range(train_folds.shape[0])], confunet.unet_im_size)*255).astype(np.uint8)
            logger.info('Extracting y')
            y = (resize_datasets([np.tile(datasets[train_folds[j]].scores[:,:,np.newaxis,:].astype(np.uint8)*255,(1,1,3,1)) for j in range(train_folds.shape[0])], confunet.unet_im_size)*255).astype(np.uint8)
            ...
            unet_my = UNetBasic.UNetBasic(confunet,
                                          dir_my_train,
                                          ims_my[0])
            ...
            logger.info('Extracting y')
            y = (resize_datasets([np.tile(datasets[train_folds[j]].gt[:,:,np.newaxis,:].astype(np.uint8)*255,(1,1,3,1)) for j in range(train_folds.shape[0])], confunet.unet_im_size)*255).astype(np.uint8)
            ...
            unet_true = UNetBasic.UNetBasic(confunet,
                                            dir_true_train,
                                            ims_true[0])
            if(resume_model is not None):
    ...
    if(pred):
        ...
        for i in range(n_folds):
            ...
            unet_true = UNetBasic.UNetBasic(confunet, dir_true_pred, ims_true[0])
            ...
            ims_train = UNetImpl.preprocess_imgs(unet_my,
                                                 im_list_train,
                                                 input_y,
                                                 input_x,
                                                 n_chans)
            ...
            preds_my = unet_my.eval(confunet,
                                    model_path_my,
                                    ims_my,
                                    mean,
                                    std)
            ...
            preds_true = unet_true.eval(confunet,
                                        model_path_true,
                                        ims_true,
                                        mean,
                                        std)
    ...
    if(score):
        for i in range(n_folds):
            ...
            gts = (resize_datasets([np.tile(datasets[pred_fold].gt[:,:,np.newaxis,:],(1,1,3,1))], confunet.unet_im_size)>0).astype(np.uint8)
...
def get_model_path(path_, ext='model-*.hdf5', take_min_loss=True):
    ...
    if(len(files) < 1):
        ...
        files = sorted([os.path.split(f)[1] for f in files])
        if(take_min_loss):
...
def get_best_model_path(path_,
                        model_fname='model-*',
                        log_fname='log.csv',
                        loss_type='binary_crossentropy'):

    loss_df = pd.read_csv(os.path.join(path_,log_fname), sep=';')
    loss = loss_df[loss_type]
    best_epoch = loss_df['epoch'][np.argmin(loss)]

    model_paths = glob.glob(os.path.join(path_,
                                         model_fname))

    model_paths = [os.path.basename(f) for f in model_paths]

    regex = r"(\d{2}(?=[^\d]))"
    saved_epochs = np.asarray([int(re.search(regex, p).group(0)) for p in model_paths])

    # Get model with closest num. of epochs
    best_model_idx = np.argmin(np.abs(np.tile(best_epoch, (saved_epochs.shape[0],1)) - saved_epochs))
    best_model_fname = model_paths[best_model_idx]

    return os.path.join(path_, best_model_fname)
