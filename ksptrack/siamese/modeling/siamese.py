...
class Siamese(nn.Module):
    ...
    def forward(self, data, edges_nn=None):
        ...
        if (edges_nn is not None):
            ...
            res.update(res_siam)
            # res['rho_hat_pooled'] = self.obj_clf(res['graph_feats'])
        # else:
        # res['rho_hat_pooled'] = torch.zeros(res['clusters'].shape[0]).to(
        # res['clusters'].device)
