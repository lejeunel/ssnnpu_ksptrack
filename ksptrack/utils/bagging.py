...
def calc_bagging_para(T,
    ...
    T_per_thread = int(T / n_jobs)
    print('Will spawn {} threads with {} trees per thread'.format(
        n_jobs, T_per_thread))
    ...
    fit_trees_partial = partial(fit_trees,
                                args=args_partial)

    thread_ids = np.arange(n_jobs)
    predict_probas = pool.map(fit_trees_partial, thread_ids)
...
def calc_bagging(T,
    ...
    with progressbar.ProgressBar(maxval=T) as bar:
        for i in range(T):
            ...
            data_bootstrap = np.concatenate(
                (data_P[bootstrap_sample_p, 2:], data_U[bootstrap_sample, 2:]),
                axis=0)
            # Train model
            model = DecisionTreeClassifier(
                max_depth=max_depth,
