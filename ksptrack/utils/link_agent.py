...
class LinkAgent:
    ...
    def make_radius_mask(self, idx):
        ...
        all_locs = [self.get_i_j(loc) for _, loc in self.locs[self.locs['frame'] == idx].iterrows()]
        for loc in all_locs:
            rr, cc = circle(
                loc[0],
                loc[1],
    ...
    def is_entrance(self, loc):
        ...
        if(self.mask_path is not None):
            return self.masks[int(loc['frame'])].astype(bool)[i, j]
        else:
    ...
    def get_closest_label(self, tl, tl_loc, labels):
        ...
        dists = [np.linalg.norm(np.array((tl_loc['x'], tl_loc['y'])) - np.array(r_compare))
                 for r_compare in [(x, y) for x, y in zip(loc_compare['x'], loc_compare['y'])]]
        loc_min = self.locs.loc[np.argmin(dists)]
        i_min, j_min = csv.coord2Pixel(loc_min['x'],
                                       loc_min['y'],
                                       self.shape[1],
                                       self.shape[0])
    ...
    def get_proba_entrance(self, tl, tl_loc, sp_desc, labels):
        ...
        if(self.mask_path is None):
            ...
            return self.get_proba(sp_desc, frame_user, label_user,
                                  frame_tl, label_tl)
        else:
            ...
            proba = np.clip(proba, a_min=self.thr_trans, a_max=1 - self.thr_trans)
            return proba
