...
base1 = GridSearchCV(RbfModelWrapper(LogisticRegression(), Xce=Xce),
                     param_grid={"gamma": np.logspace(-1, 1, 6)},
                     scoring=scorer)
base2 = GridSearchCV(RbfModelWrapper(LogisticRegression(), Xce=Xce),
                     param_grid={"gamma": np.logspace(-1, 1, 6)},
                     scoring=scorer)

clf = PUAdapter(base1).fit(X, s)
trad_clf = base2.fit(X, s)
print ("accuracy (PU):", metrics.accuracy_score(y[s == 0],
                                               clf.predict(X[s == 0])))
