...
def draw2DPoint(locs2d, frame_num, img, radius=2, color=(0, 255, 0)):
    ...
    if(frame_num in locs2d[:, 0]):
        ...
        i,j = coord2Pixel(locs2d[frame_row,3],
                          locs2d[frame_row,4],
                          width,
                          height)
...
def pixCoord2SPcentroid(coords_arr,labels):
    ...
    for i in range(coords_arr.shape[0]):
        this_centroid = ndimage.measurements.center_of_mass(labels[i,:,:] == labels[i,coords_arr[i,4],coords_arr[i,3]])
        centroids_out[i,...] = (coords_arr[i,0:3],this_centroid[1],this_centroid[0])
...
def readCsv(filePath):
    ...
    return np.loadtxt(open(filePath, "rb"),
                      delimiter=";",
                      skiprows=5)
