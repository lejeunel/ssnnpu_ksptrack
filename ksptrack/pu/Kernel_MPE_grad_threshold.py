...
def find_nearest_valid_distribution(u_alpha, kernel, initial=None, reg=0):
    ...
    distance_sqd = solution['primal objective'] + np.dot(
        u_alpha.T, np.dot(kernel, u_alpha))[0, 0]
...
def compute_best_rbf_kernel_width(X_mixture, X_component):
    ...
    for kernel_width in kernel_width_vals:
        ...
        dist_diff = np.concatenate((np.ones((N, 1)) / N, -1 * np.ones(
            (M, 1)) / M))
...
if __name__ == '__main__':
    ...
    X_mixture = np.concatenate(
        (np.random.randn(int((1 - kappa_star) * tot_size / 2), 2),
         np.random.randn(int(kappa_star * tot_size / 2), 2) + mean_comp))
