...
def df_to_tgt(df):
    ...
    target_aug = copy.deepcopy(target_pos)

    for _, r in df.iterrows():
        if not np.isnan(r['label']):
            if r['from_aug']:
                target_aug[r['frame']][int(r['label'])] = 1.
            else:
...
def make_edges_ccl(clusters,
    ...
    for data in tqdm(dataloader):
        ...
        clst = np.concatenate([clusters[i] for i in data['frame_idx']])
        ...
        nodes_ = torch.stack((nodes_[0][idx], nodes_[1][idx],
                              torch.from_numpy(clst[nodes_[0][idx]])))
        if nodes_.shape[1] != np.unique(all_edges).shape[0]:
            missing_nodes = [
                n for n in np.unique(all_edges) if n not in nodes_[0]
            ]
            max_cc = nodes_[1].max() + 1
            for i, n in enumerate(missing_nodes):
                nodes_ = torch.cat((nodes_, torch.tensor(
                    (n, max_cc, clst[n]))[..., None]),
                                   axis=1)
                max_cc += 1
