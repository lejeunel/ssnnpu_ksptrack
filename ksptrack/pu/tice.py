...
def main():
    parser = argparse.ArgumentParser(
        description='Estimate the class prior through decision tree induction.'
    )
    ...
    parser.add_argument(
        ...
        type=str,
        help=
        'Path to the folds, if no folds are provided, 5 random folds are chosen.'
    )
    parser.add_argument("-d",
                        '--delta',
                        default=None,
                        type=float,
                        help='Delta, default: using formula from paper.')
    parser.add_argument("-k",
                        '--max-bepp',
                        default=5,
                        type=int,
                        help='The max-bepp parameter k, default=5.')
    parser.add_argument(
        "-M",
        '--maxSplits',
        default=500,
        type=int,
        help='The maximum number of splits in the decision tree, default=500.')
    parser.add_argument(
        '--promis',
        action="store_true",
        help=
        "Set this option to only use the most promising subset (instead of calculating the maximum lower bound)"
    )
    parser.add_argument('--delimiter',
                        default=',',
                        type=str,
                        help="Delimiter of the data files")
    parser.add_argument(
        "-m",
        '--minT',
        default=10,
        type=int,
        help='The minimum set size to update the lower bound with, default=10.'
    )
    parser.add_argument(
        "-i",
        '--nbIts',
        ...
        type=int,
        help=
        "The number of times to repeat the the estimation process. Default 2 (first with c_prior=0.5, then with c_prior=c_estimate)"
    )
    ...
    ti = time.time()
    (c_estimate, c_its_estimates) = tice(data,
                                         labels,
                                         args.max_bepp,
                                         folds,
                                         args.delta,
                                         nbIterations=args.nbIts,
                                         maxSplits=args.maxSplits,
                                         useMostPromisingOnly=args.promis,
                                         minT=args.minT)
    ti = time.time() - ti
    ...
    if args.out:
        ...
        outfile.write("\n")
        for (it, c_estimates) in zip(range(1, args.nbIts + 1),
                                     c_its_estimates):
            outfile.write("c_estimates it " + str(it) + ":\t" +
                          str(c_estimates) + "\n")
        outfile.write("\n")
...
def max_bepp(k):
    def fun(counts):
        return max(
            list(
                map(
                    lambda T_P: (0 if T_P[0] == 0 else float(T_P[1]) /
                                 (T_P[0] + k)), counts)))
...
def tice(data,
    ...
    for it in range(nbIterations):
        ...
        for (tree_train, estimate) in generate_folds(folds):
            print('iter {}/{}. fold {}/{}'.format(it + 1, nbIterations, f,
                                                  np.unique(folds).shape[0]))
            ...
            if useMostPromisingOnly:
                ...
                for tree_subset, estimate_subset in subsetsThroughDT(
                        ...
                        delta=cur_delta,
                        n_splits=n_splits):
                    tree_est_here = low_c(tree_subset,
                                          labels,
                                          cur_delta,
                                          1,
                                          c=c_estimate)
                ...
                for tree_subset, estimate_subset in subsetsThroughDT(
                        ...
                        delta=cur_delta,
                        n_splits=n_splits):
                    est_here = low_c(estimate_subset,
                                     labels,
                                     cur_delta,
                                     minT,
                                     c=c_estimate)
...
def subsetsThroughDT(data,
    ...
    heapq.heappush(priorityq,
                   (-low_c(tree_train, labels, delta, 0, c=c_prior),
                    -(tree_train & labels).count(), tree_train, estimate,
    ...
    while n < maxSplits and len(priorityq) > 0:
        n += 1
        (ppos, neg_lab_count, subset_train, subset_estimate, available,
         depth) = heapq.heappop(priorityq)
        ...
        for a in available:
            ...
            subsets_estimate = list(
                map(lambda X_cond: X_cond & subset_estimate,
                    conditionSets[a]))  # X_cond & subset_train
            estimate_lab_counts = list(
                map(lambda subset: (subset & labels).count(),
                    subsets_estimate))
        ...
        fake_split = len(
            list(
                filter(lambda subset: subset.count() > 0,
                       best_subsets_estimate))) == 1

        if best_score > 0 and not fake_split:
            newAvailable = available - {best_a} - uselessAs
            for subsub_train, subsub_estimate in zip(best_subsets_train,
                                                     best_subsets_estimate):
                yield (subsub_train, subsub_estimate)
            minimumLabeled = c_prior * (1 - c_prior) * (1 - delta) / (
                delta * (1 - c_cur_best)**2)

            for (subsub_lab_count, subsub_train,
                 subsub_estimate) in zip(best_lab_counts, best_subsets_train,
                                         best_subsets_estimate):
                        ...
                        if lab_count != 0 and lab_count != total:  # stop criterion: purity
                            heapq.heappush(
                                ...
                                (-low_c(
                                 ...
                                 -train_lab_count, subsub_train,
                                 subsub_estimate, newAvailable, depth + 1))
...
def tice_wrapper(data,
    ...
    c = tice(data,
             target,
             k,
             np.random.randint(n_folds, size=len(data)),
             delta=delta,
             maxSplits=maxSplits,
             minT=10,
             nbIterations=n_iter,
...
def min_max_scale(data):
    ...
    data_norm = data_norm / (data_norm.max(axis=0) + 1e-8)
    return data_norm
