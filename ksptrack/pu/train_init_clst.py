...
def train_kmeans(model,
    ...
    res = clst.get_features(model, dataloader, device)
    ...
    if (comp_probas == 'pu'):
        ...
        probas = np.concatenate(res['obj_preds'])
    else:
        probas = None
    ...
    predictions = np.split(predictions,
                           np.cumsum([len(f) for f in res['feats']]))[:-1]
...
def train(cfg, model, device, dataloaders, run_path=None):
    ...
    path = pjoin(run_path, 'checkpoints', 'init_dec.pth.tar')
