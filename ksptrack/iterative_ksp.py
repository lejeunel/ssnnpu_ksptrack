...
def main(cfg):
    ...
    if (cfg.monitor_score):
        ...
        gtFileNames = utls.makeFrameFileNames(
            cfg.frame_prefix, cfg.frameDigits, cfg.truth_dir,
            cfg.root_path, cfg.ds_dir, cfg.frame_extension)
    ...
    entrance_agent = EntranceAgent(cfg.entrance_mask_path)
    ...
    while ((find_new_forward or find_new_backward) and (i < cfg.n_iters_ksp)):
        ...
        if ((i > 0) & find_new_forward & dm.cfg.do_temporal_merge):
            g_for.merge_tracklets_temporally(pos_tls_for, dm.fg_pm_df,
                                             dm.sp_desc_df, dm.cfg.pm_thr)

        if ((i > 0) & find_new_backward & dm.cfg.do_temporal_merge):
        ...
        if (find_new_backward):
            ...
            g_back.makeFullGraph(
                ...
                dm.cfg.norm_neighbor_in,
                dm.cfg.thresh_aux_fix,
                dm.cfg.hoof_tau_u,
                direction='backward',
                labels=dm.labels)
        ...
        if ((find_new_forward or find_new_backward)):
            ...
            if (i + 1 < cfg.n_iters_ksp):
                ...
                dm.cfg.thresh_aux.append(cfg.thresh_aux_fix)
