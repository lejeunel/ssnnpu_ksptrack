...
df_18 = df_18.set_index(['Types', 'Methods']).rename(columns={'F1 mean': 'F1',
                                                              'F1 std': '_F1',
                                                              'PR mean': 'PR',
                                                              'PR std': '_PR',
                                                              'RC mean': 'RC',
...
df_all = df_all.rename(index={'KSP': 'KSPTrack'})
df_all = df_all.rename(index={'lfda': 'KSPTrack/GMM'})
...
def myformat(r):
    if(r['bold'].iat[0]):
        r['F1'] = '$\\bm{' + r['F1'].apply(str) + '} \pm ' + r['_F1'].apply(str) + '$'
    else:
